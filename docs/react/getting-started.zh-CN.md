...
import { LocaleProvider, DatePicker, message } from 'de-dashboard';
...
```diff
- import { LocaleProvider, DatePicker, message } from 'de-dashboard';
+ import { LocaleProvider, DatePicker, message, Alert } from 'de-dashboard';
```
...
如果你在开发环境的控制台看到下面的提示，那么你可能使用了 `import { Button } from 'de-dashboard';` 的写法引入了 antd 下所有的模块，这会影响应用的网络性能。
